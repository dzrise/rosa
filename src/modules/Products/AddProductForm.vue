<template>
  <base-form :validators-collection="refs" @on-submit="addProduct">
    <template v-slot:form-content>
      ...
      <button type="submit" class="btn btn-primary mr-3" title="Добавить">Добавить</button>
    </template>
  </base-form>
</template>
...
export default {
  ...
  setup(props) {
    ...
    const addProduct = async () => {
      const submittedData = {
        name: {ru: inputName.value},
        // name: inputName.value,
        // description: inputPrice.value,
        description: {description: inputPrice.value},
      }

      await store.dispatch('products/addProduct', submittedData);
    }
