<template>
  <base-form :validators-collection="refs" @on-submit="handlerEditHotel">
    <template v-slot:form-content>
      <h1 class="mb-3">Изменить отель</h1>
      <h3 class="mb-5"><b>ID:</b>{{ hotel.id }}</h3>
      <div class="mb-5">
      ...
      <button type="submit" class="btn btn-primary mt-5" title="Изменить отель">Изменить отель</button>
    </template>
  </base-form>
</template>
...
export default {
  ...
  setup({locations, hotelTypes, hotelBrands, hotel}) {
    ...
    const starCount = reactive({
      id: 'starCount',
      value: {id: hotel.starCount || '', name: hotel.starCount || ''},
      options: [
        {id: 1, name: 1, value: 1},
        {id: 2, name: 2, value: 2},
        {id: 3, name: 3, value: 3},
    ...
    const location = reactive({
      id: 'location',
      value: {id: hotel.location?.id || '', name: hotel.location?.name?.ru || ''},
    ...
    async function handlerEditHotel() {
