<template>
  <base-form :validators-collection="refs" @on-submit="editCategory">
...
export default {
  ...
  components: {
    ...
    VTextInput,
    BaseForm,
    DefaultButton, UploadInput, BaseSelect, MultipleSelect
  },
  props: {
  ...
  setup(props) {
    ...
    const color1 = ref(props.category?.webGradientColor1 || '#ffffff');
    const color2 = ref(props.category?.webGradientColor2 || '#333333');

    const nameRu = reactive({
      id: 'nameRu',
      value: props.category.name?.ru || '',
      required: true,
    ...
    const descriptionRu = reactive({
      id: 'descriptionRu',
      value: props.category.description.ru,
      required: true,
    ...
    const descriptionEn = reactive({
      id: 'descriptionEn',
      value: props.category.description.en,
      required: true,
      placeholder: 'Описание категории',
      errors: {
        required: 'Поле описание категории обязательно для заполнения\n'
    ...
    const editCategory = async () => {
      const submitData = {name: 22}
      console.log(submitData)
      //   const {id} = props.category;
      //   const data = {
      //     id,
      //     submitData: {
      //       name: inputNameRu.value,
      //       description: inputDescriptionRu.value,
      //     }
      //   }
      //   await store.dispatch('categories/editCategory', data)
    }
    return {
      refs, addRef,
      color1, color2,
